...
The plane-dihedral representation offers several advantages over existing spinor visualization methods:
...
\end{itemize}
The plane-dihedral representation retains all spinorial degrees of freedom while still displaying the Bloch vector as the normal to the rotated equatorial plane.
...
Some authors visualize spinors using oriented objects (arrows with flags, or twisted ribbons) that acquire a twist under $2\pi$ rotations. Our approach is similar in spirit but more systematic: the plane-dihedral circles and their phase markers provide a complete, easily parsable representation.

\subsection{Physical interpretation}
\label{sec:disc_physics}

The plane-dihedral construction has a natural interpretation in terms of quantum measurement and state geometry:
\begin{itemize}
...
While the three-dimensional plane-dihedral representation provides complete geometric information, it can be challenging to interpret in static printed figures where interactive exploration is not possible. For such cases, we provide a two-dimensional projection onto the equatorial ($xy$) plane.
...
\begin{itemize}
    ...
    \item The \textbf{equator contact points} mark where each plane-dihedral circle meets the equator, encoding the azimuthal phase $\phi$. The upper face contact point $P_\uparrow$ is marked with an upward-pointing triangle ($\blacktriangle$) in teal/bluish-green, and the lower face contact point $P_\downarrow$ with a downward-pointing triangle ($\blacktriangledown$) in orange;
    \item The \textbf{plane-dihedral circles} are visually distinguished by both color and line thickness: $c_\uparrow$ is drawn with a thicker line in teal/bluish-green and $c_\downarrow$ with a thinner line in orange;
...
\begin{enumerate}
    \item \textbf{Circle mode} (default): The plane-dihedral faces are drawn as circles with their true 3D radii. This provides the clearest visualization of the probability amplitudes, as the disk areas directly encode the Born-rule probabilities.
    \item \textbf{Ellipse mode}: The plane-dihedral faces are drawn as ellipses, representing their true orthogonal projection onto the $xy$-plane. The major axis equals the plane-dihedral radius, while the minor axis is scaled by $\cos(\beta)$ where $\beta$ is the tilt angle of the face plane.
\end{enumerate}

Figure~\ref{fig:2d_projection} illustrates both modes for a general spinor state. In circle mode, the two circles touch at the projection of their shared 3D contact point, preserving the tangency condition while displaying the true plane-dihedral radii. In ellipse mode, the projected ellipses show the geometric distortion from the inclined 3D planes.
...
\begin{figure}[ht!]
    ...
    \caption{Two-dimensional projection of the plane-dihedral visualization for the spinor $\ssp = [\cos(\pi/6), \sin(\pi/6)e^{i\pi/4}]^T$. (a)~Circle mode: the plane-dihedral faces are drawn as tangent circles with their true 3D radii. (b)~Ellipse mode: the plane-dihedral faces are drawn as their orthogonal projections onto the $xy$-plane.}
...
The plane-dihedral visualization is particularly effective for animating spinor dynamics. The reference implementation (Section~\ref{sec:algorithm} and Appendix~\ref{sec-app:pseudocode}), provided as the open\nobreakdash-source Python package \texttt{spinor\_viz}~\cite{spinor_viz_github}, supports frame-by-frame animation of:
\begin{itemize}
    \item Rotations about arbitrary axes, showing how the plane-dihedral circles smoothly transform;
...
Figure~\ref{fig:z_rotation} illustrates the evolution of a spinor under rotation about the $z$-axis. The initial state has $\theta=\pi/3$ and $\phi=\pi/4$. As the rotation angle increases from $0$ to $2\pi$, the plane-dihedral circles rotate about their respective axes, demonstrating the half-angle relationship: a $2\pi$ physical rotation corresponds to the spinor acquiring a phase of $\pi$, visually manifested as a $\pi$ rotation of the in-face marker orientations.
...
\begin{figure}[ht!]
    ...
    \caption{Animation sequence of a spinor undergoing rotation about the $z$-axis. Starting from $\theta=\pi/3$, $\phi=\pi/4$, the spinor is rotated by angle $\omega$ via the operator $e^{-i\omega\sigma_z/2}$. After a $2\pi$ rotation (frame e), the spinor has returned to the same Bloch vector but acquired a $\pi$ phase shift, visible in the plane-dihedral marker orientations. Full return to the initial state requires a $4\pi$ rotation.}
...
\begin{figure}[ht!]
    ...
    \caption{2D ellipse-mode projection of the $z$-rotation sequence from Fig.~\ref{fig:z_rotation}. The projected ellipses show the true orthogonal projection of the inclined plane-dihedral circles onto the $xy$-plane.}
...
\begin{figure}[ht!]
    ...
    \caption{2D circle-mode projection of the $z$-rotation sequence from Fig.~\ref{fig:z_rotation}. The tangent circles preserve the true plane-dihedral radii, with their areas encoding the Born-rule probabilities.}
...
We have presented a novel geometric construction for visualizing two-dimensional spinors. The plane-dihedral representation:
\begin{enumerate}
