...
The reference Python implementation, available as an open-source package at \url{https://github.com/al-rigazzi/spinor_viz}, uses a colorblind-friendly palette (the Wong palette~\cite{wong2011points}) and distinct visual encodings to distinguish the two hyperchord circles.
Specifically:
\begin{itemize}
    \item The \emph{upper hyperchord} $c_\uparrow$ is drawn with a thicker line (linewidth~3.0) in teal/bluish-green (\texttt{\#009E73}), and its kissing point with the equator $P_\uparrow$ is marked with an upward-pointing triangle ($\blacktriangle$).
    \item The \emph{lower hyperchord} $c_\downarrow$ is drawn with a thinner line (linewidth~1.5) in orange (\texttt{\#E69F00}), and its kissing point with the equator $P_\downarrow$ is marked with a downward-pointing triangle ($\blacktriangledown$).
...
\end{itemize}
These color and thickness choices ensure that the two hyperchord circles remain distinguishable both in color and in achromatic reproductions.


\subsection{Main visualization routine}

The main routine takes a spinor $\ssp \in \mathbb{C}^2$ and renders the complete hyperchord visualization.

\medskip
\noindent\textbf{Procedure: PlotSpinor3D}\\
\textbf{Input:} Spinor $\ssp = [\spu, \spd]^T$, view direction (optional)\\
\textbf{Output:} 3D plot of hyperchord visualization
...
\item \textit{Draw upper hyperchord circle if $r_\uparrow>\epsilon$:}
\begin{itemize}
...
\item \textit{Draw lower hyperchord circle if $r_\downarrow>\epsilon$:}
\begin{itemize}
